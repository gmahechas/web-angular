...
export class FormPageOfficeComponent implements OnInit {
  ...
  onUserOffice(office: Office) {
    ...
    this.store.dispatch(new fromCore.Go({
      path: ['office', office.office_id, { outlets: { 'router-outlet-office': ['user-office', 'office', office.office_id] } }]
    }));
  }
  ...
  onOfficeDepartment(office: Office) {
    this.store.dispatch(new fromCore.Go({
      path: ['office', office.office_id, {
        outlets: { 'router-outlet-office': ['office-department', 'office', office.office_id] }
      }]
    }));
  }
}
